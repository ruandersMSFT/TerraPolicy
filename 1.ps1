    ...
    if ($policyAssignments.Count -gt 0) {
        ...
        foreach ($policyAssignment in $policyAssignments) {
            ...
            foreach ($managementGroupException in $managementGroups) {
                ...
                $managementGroupPolicyExemptions = Get-AzPolicyExemption -PolicyAssignmentIdFilter $policyAssignment.Id -Scope $managementGroupException.Id | Where-Object { $_.Id -like "$($managementGroupException.Id)*" }
            ...
            foreach ($subscriptionException in $subscriptions) {
                ...
                $subscriptionPolicyExemptions = Get-AzPolicyExemption -PolicyAssignmentIdFilter $policyAssignment.Id -Scope "/subscriptions/$($subscriptionException.Id)" | Where-Object { $_.Id -like "/subscriptions/$($subscriptionException.Id)*" }
